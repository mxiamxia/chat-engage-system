    ...
    _onLogin: (data, headers) =>
        if data
            if not data.id
                ...
                @token = headers.token
#                @socketUrl = 'ws://' + @host + (if @options.wssPort? then ':'+ @options.wssPort else ':443') + '/api/v1/websocket'
                if @host.indexOf 'https'
                  ...
                  wsUrl = 'wss://'
                @socketUrl = wsUrl + hostUrl + '/api/v1/websocket'
#                @socketUrl = 'ws://http://www.cyberobject.com:8065/api/v1/websocket'
    ...
    _apiCall: (method, path, params, callback) ->
      ...
      console.log 'full url=' + @host + '/api/v3' + path + '==='
      options ={
        uri: @host + '/api/v1' + path,
        method: method,
        json: params,
        headers:{
...
#        post_data = JSON.stringify(params) if params?
#        console.log 'post data=' + post_data
#        console.log 'full url=' + @host + '/api/v1' + path
#        options =
#            hostname: @host
#            method: method
#            path: '/api/v1' + path
